...
export default function Game11Litres({ onComplete, onBack }) {
  ...
  const gameLoop = useGameLoop(useCallback(({ elapsed, delta }) => {
    ...
    if (phase === 'ready') {
      ...
      ctx.moveTo(0, waveY);
      for (let x = 0; x <= w; x += 4) {
        const y = waveY + Math.sin(x * 0.02 + elapsed * 2) * 12 + Math.sin(x * 0.01 + elapsed) * 8;
        ctx.lineTo(x, y);
      }
      ctx.lineTo(w, h);
      ctx.lineTo(0, h);
      ...
      ctx.fillText("A pelican's pouch holds", cx, h * 0.35);
      ctx.fillText('11 litres of water!', cx, h * 0.35 + 26);
      ctx.font = '15px sans-serif';
      ctx.fillStyle = COLORS.gray;
      ctx.fillText('SWIPE DOWN to scoop water', cx, h * 0.48);
      ctx.fillText('SWIPE UP to fly higher', cx, h * 0.48 + 24);
      ctx.fillStyle = COLORS.white;
    ...
    // Check scoop: pelican near wave surface
    const waveSurface = h * 0.78;
    if (s.scooping && s.pelicanY > waveSurface - 30 && s.scoopCooldown <= 0 && s.litres < TARGET_LITRES) {
      // Calculate wave height at pelican position
      const waveVal = Math.sin(s.pelicanX * 0.02 + s.wavePhase);
      const timing = (waveVal + 1) / 2; // 0..1, 1 = wave peak
      const scoopAmount = 0.5 + timing * 1.5; // 0.5 to 2.0
      s.litres = Math.min(TARGET_LITRES, s.litres + scoopAmount);
      s.lastScoopAmount = scoopAmount;
      s.scoopFlash = 0.5;
      s.scoopCooldown = 0.6;
      s.totalScoops++;
      if (scoopAmount >= 1.5) s.perfectScoops++;
      s.targetAlt = 0.35;
      s.scooping = false;

      spawnParticles(s.pelicanX, s.pelicanY + 15, 8, [
        [0, 191, 255], [0, 150, 220], [255, 255, 255],
      ]);
      sounds.splash();
    }
    ...
    for (const g of s.seagulls) {
      ...
      g.x -= g.speed * delta;
      g.wingPhase += delta * 8;
      // Collision with pelican
      const dx = g.x - s.pelicanX;
      const dy = g.y - s.pelicanY;
      if (Math.abs(dx) < 35 && Math.abs(dy) < 25) {
        ...
        spawnParticles(s.pelicanX, s.pelicanY, 6, [
          [239, 68, 68], [255, 150, 100], [255, 255, 255],
        ]);
        sounds.tick();
        continue;
      }
    ...
    if (s.timeLeft <= 0 && phase === 'playing') {
      ...
      const score = Math.round(s.litres * speedBonus * 10);
      setPhase('ended');
    ...
    // Ocean waves (multiple sin layers)
    const waveBase = h * 0.78;
    for (let layer = 0; layer < 3; layer++) {
      const layerAlpha = 0.6 + layer * 0.15;
      const layerOffset = layer * 6;
      ctx.beginPath();
      ctx.moveTo(0, waveBase + layerOffset);
      for (let x = 0; x <= w; x += 3) {
        const y = waveBase + layerOffset
    ...
    // Eye
    ctx.beginPath();
    ctx.arc(35, -12, 2.5, 0, Math.PI * 2);
    ctx.fillStyle = '#000';
    ctx.fill();
    ctx.beginPath();
    ctx.arc(35.5, -12.5, 1, 0, Math.PI * 2);
    ctx.fillStyle = '#fff';
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(40, 2);
    ctx.quadraticCurveTo(52, 8 + pouchFill * 10, 65, -2);
    ctx.strokeStyle = '#E8941A';
    ctx.lineWidth = 1.5;
    ctx.stroke();
    if (pouchFill > 0) {
      ctx.fillStyle = `rgba(0,150,220,${0.4 + pouchFill * 0.4})`;
      ctx.fill();
    }
    ...
    for (const g of s.seagulls) {
      ...
      ctx.beginPath();
      ctx.moveTo(0, 0);
      ctx.quadraticCurveTo(-8, -10 + sw, -20, -5 + sw * 0.5);
      ctx.moveTo(0, 0);
      ctx.quadraticCurveTo(8, -10 + sw, 20, -5 + sw * 0.5);
      ctx.strokeStyle = '#555';
      ctx.lineWidth = 2.5;
    ...
    if (s.scoopFlash > 0.05) {
      ...
      ctx.fillText(
        isPerfect ? `+${s.lastScoopAmount.toFixed(1)}L PERFECT!` : `+${s.lastScoopAmount.toFixed(1)}L`,
        px, py - 40 - (1 - s.scoopFlash) * 30
      );
      ctx.restore();
    }
    ...
    ctx.stroke();
    // Fill
    const fillFrac = Math.min(1, s.litres / TARGET_LITRES);
    const fillH = gaugeH * fillFrac;
    const fillGrad = ctx.createLinearGradient(0, gaugeTop + gaugeH - fillH, 0, gaugeTop + gaugeH);
    fillGrad.addColorStop(0, '#00BFFF');
    fillGrad.addColorStop(1, '#0077B6');
    ctx.fillStyle = fillGrad;
    ctx.beginPath();
    ctx.roundRect(gaugeX - gaugeW / 2 + 2, gaugeTop + gaugeH - fillH + 2, gaugeW - 4, fillH - 4, 4);
    ctx.fill();
    // Label
    ctx.font = 'bold 16px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillStyle = COLORS.white;
    ctx.fillText(`${s.litres.toFixed(1)}`, gaugeX, gaugeTop - 8);
    ctx.font = '11px sans-serif';
    ctx.fillStyle = COLORS.cyan;
    ctx.fillText(`/ ${TARGET_LITRES}L`, gaugeX, gaugeTop - 8 + 16);
