...
function drawToucan(ctx, x, y, rx, ry, bodyColor, beakColor, alpha) {
  ...
  ctx.beginPath(); ctx.ellipse(0, 0, rx, ry, 0, 0, Math.PI * 2); ctx.fillStyle = bodyColor; ctx.fill();
  ctx.strokeStyle = 'rgba(0,0,0,0.3)'; ctx.lineWidth = 1; ctx.stroke();
  ctx.beginPath(); ctx.ellipse(2, 2, rx * 0.6, ry * 0.7, 0, 0, Math.PI * 2); ctx.fillStyle = '#FFF8E1'; ctx.fill();
  ctx.beginPath(); ctx.arc(rx * 0.7, -ry * 0.4, ry * 0.55, 0, Math.PI * 2); ctx.fillStyle = '#111'; ctx.fill();
  ctx.beginPath(); ctx.arc(rx * 0.8, -ry * 0.5, 2, 0, Math.PI * 2); ctx.fillStyle = '#fff'; ctx.fill();
  ctx.beginPath(); ctx.arc(rx * 0.85, -ry * 0.52, 1, 0, Math.PI * 2); ctx.fillStyle = '#000'; ctx.fill();
  ctx.beginPath(); ctx.moveTo(rx * 0.95, -ry * 0.3);
  ctx.quadraticCurveTo(rx * 1.6, -ry * 0.5, rx * 1.7, -ry * 0.2);
  ctx.quadraticCurveTo(rx * 1.5, ry * 0.1, rx * 0.9, 0); ctx.closePath();
  ctx.fillStyle = beakColor; ctx.fill();
  ctx.beginPath(); ctx.arc(rx * 1.65, -ry * 0.2, 2.5, 0, Math.PI * 2); ctx.fillStyle = '#333'; ctx.fill();
  ctx.restore();
}
...
export default function GameDortoirA6({ onComplete, onBack }) {
  ...
  const makeToucans = useCallback(() => Array(NUM_TOUCANS).fill(null).map((_, i) => {
    ...
    return {
      id: i, order, rx: 18 + order * 5, ry: 12 + order * 3,
  ...
  const state = useRef({
    toucans: [], temperature: 18, timeLeft: GAME_DURATION,
    placedCount: 0, dragging: null, dragOffX: 0, dragOffY: 0,
    holeX: 0, holeY: 0, holeRadius: 60,
    windTimer: WIND_INTERVAL, windActive: false, windAlpha: 0, windDir: 1,
    wrongFlash: 0, correctFlash: 0, layoutDone: false, slots: [],
  ...
  const spawnParticles = useCallback((cx, cy, count, colors) => {
    ...
    for (let i = 0; i < s.particles.length && spawned < count; i++) {
      ...
      if (!p.active) {
        p.active = true;
        p.x = cx + (Math.random() - 0.5) * 24; p.y = cy + (Math.random() - 0.5) * 16;
        const angle = Math.random() * Math.PI * 2; const speed = 30 + Math.random() * 80;
        p.vx = Math.cos(angle) * speed; p.vy = Math.sin(angle) * speed;
        p.life = 0.3 + Math.random() * 0.5; p.maxLife = p.life;
        const c = colors[Math.floor(Math.random() * colors.length)];
        p.r = c[0]; p.g = c[1]; p.b = c[2];
        p.size = 1.5 + Math.random() * 3; p.type = Math.random() > 0.5 ? 'dot' : 'line';
        spawned++;
      }
    }
  }, []);
  ...
  const layoutPositions = useCallback((w, h) => {
    ...
    s.holeX = w / 2; s.holeY = h * 0.4; s.holeRadius = Math.min(w, h) * 0.12;
    const positions = [
    ...
    s.slots = angles.map(a => ({ x: s.holeX + Math.cos(a) * s.holeRadius * 0.35, y: s.holeY + Math.sin(a) * s.holeRadius * 0.35 }));
    for (let i = 0; i < NUM_TOUCANS; i++) {
      const t = s.toucans[i]; if (!t.placed) { t.homeX = positions[i].x; t.homeY = positions[i].y; t.x = t.homeX; t.y = t.homeY; }
    }
    s.layoutDone = true;
  }, []);

  const tryPickup = useCallback((x, y) => {
    const s = state.current; let best = null, bestDist = 60;
    for (const t of s.toucans) {
      if (t.placed) continue;
      const d = Math.sqrt((x - t.x) ** 2 + (y - t.y) ** 2);
      if (d < bestDist) { bestDist = d; best = t; }
    }
    if (best) { s.dragging = best; s.dragOffX = best.x - x; s.dragOffY = best.y - y; }
  }, []);

  const tryDrop = useCallback(() => {
    const s = state.current; if (!s.dragging) return;
    const t = s.dragging; s.dragging = null;
    const dist = Math.sqrt((t.x - s.holeX) ** 2 + (t.y - s.holeY) ** 2);
    if (dist < s.holeRadius + 15) {
      const expected = NUM_TOUCANS - s.placedCount;
      if (t.order === expected) {
        t.placed = true; t.slotIndex = s.placedCount; t.x = s.slots[s.placedCount].x; t.y = s.slots[s.placedCount].y;
        s.placedCount++; s.temperature = Math.min(TARGET_TEMP, s.temperature + (TARGET_TEMP - 18) / NUM_TOUCANS);
        s.correctFlash = 0.4;
        spawnParticles(s.holeX, s.holeY, 10, [[46,234,163],[245,166,35],[255,255,255]]);
        sounds.chime();
      } else {
        t.popAnim = 1.0; t.x = t.homeX; t.y = t.homeY; s.wrongFlash = 0.4;
        spawnParticles(s.holeX, s.holeY, 6, [[239,68,68],[255,100,100],[255,200,200]]);
        sounds.tick();
      }
    } else { t.x = t.homeX; t.y = t.homeY; }
  ...
  const handleDrag = useCallback((info) => {
    const s = state.current;
    if (s.dragging) { s.dragging.x = info.x + s.dragOffX; s.dragging.y = info.y + s.dragOffY; }
  }, []);

  const handleSwipe = useCallback(() => { tryDrop(); }, [tryDrop]);

  useTouch(canvasRef, { onTap: handleTap, onDrag: handleDrag, onSwipe: handleSwipe });

  // Direct pointer handling for responsive drag
  useEffect(() => {
    const el = canvasRef.current; if (!el) return;
    const ps = { down: false };
    const onDown = (e) => { ps.down = true; if (phase === 'playing') tryPickup(e.clientX, e.clientY); };
    const onMove = (e) => { if (!ps.down) return; const s = state.current; if (s.dragging) { s.dragging.x = e.clientX + s.dragOffX; s.dragging.y = e.clientY + s.dragOffY; } };
    const onUp = () => { if (ps.down) { ps.down = false; tryDrop(); } };
  ...
  const gameLoop = useGameLoop(useCallback(({ elapsed, delta }) => {
    ...
    const ctx = canvas.getContext('2d'); ctx.save(); ctx.scale(dpr, dpr);
    const s = state.current; const cx = w / 2;
    if (!s.layoutDone) layoutPositions(w, h);

    if (phase === 'ready') {
      const bg = ctx.createLinearGradient(0, 0, 0, h);
      bg.addColorStop(0, '#0B132B'); bg.addColorStop(0.6, '#1C2541'); bg.addColorStop(1, '#3A506B');
      ctx.fillStyle = bg; ctx.fillRect(0, 0, w, h);
      ctx.fillStyle = COLORS.white; ctx.font = 'bold 26px sans-serif'; ctx.textAlign = 'center';
      ctx.fillText('Dortoir \u00e0 6', cx, h * 0.22);
      ctx.font = '17px sans-serif'; ctx.fillStyle = COLORS.gold;
      ctx.fillText('Toucans sleep 6 in a tree hole to stay warm!', cx, h * 0.32);
      ctx.font = '14px sans-serif'; ctx.fillStyle = COLORS.gray;
      ctx.fillText('DRAG toucans \u2022 Biggest first, smallest last', cx, h * 0.45);
    ...
    if (s.windTimer <= 0) {
      ...
      if (s.placedCount < NUM_TOUCANS) {
        s.temperature = Math.max(10, s.temperature - 5); s.windActive = true; s.windAlpha = 1.0;
        s.windDir = Math.random() > 0.5 ? 1 : -1; sounds.whoosh();
      }
    }
    if (s.windActive) { s.windAlpha -= delta * 1.5; if (s.windAlpha <= 0) { s.windActive = false; s.windAlpha = 0; } }
    s.wrongFlash *= Math.pow(0.01, delta); s.correctFlash *= Math.pow(0.01, delta);
    for (const t of s.toucans) { if (t.popAnim > 0) t.popAnim -= delta * 3; t.wobble = Math.sin(elapsed * 3 + t.id * 1.5) * 2; }
    for (const p of s.particles) { if (!p.active) continue; p.x += p.vx * delta; p.y += p.vy * delta; p.vy += 40 * delta; p.life -= delta; if (p.life <= 0) p.active = false; }

    if ((s.timeLeft <= 0 || s.placedCount === NUM_TOUCANS) && phase === 'playing') {
      const speedBonus = Math.max(1, 1 + s.timeLeft / GAME_DURATION);
      setPhase('ended'); setDisplayScore(Math.round(s.temperature * speedBonus * s.placedCount)); return;
    }

    // --- RENDER ---
    const bg = ctx.createLinearGradient(0, 0, 0, h);
    bg.addColorStop(0, '#0B132B'); bg.addColorStop(0.5, '#1C2541'); bg.addColorStop(1, '#2A3A50');
    ctx.fillStyle = bg; ctx.fillRect(0, 0, w, h);
    ...
    for (let i = 0; i < 8; i++) { const ly = h * 0.2 + i * h * 0.06; ctx.beginPath(); ctx.moveTo(cx - tw / 2 + 5, ly); ctx.quadraticCurveTo(cx, ly + 5, cx + tw / 2 - 5, ly - 3); ctx.stroke(); }
    // Branches
    ...
    // Placed toucans
    for (const t of s.toucans) { if (t.placed) drawToucan(ctx, t.x, t.y + t.wobble, t.rx * 0.7, t.ry * 0.7, t.color, t.beakColor, 0.85); }
    ...
    for (const t of s.toucans) {
      ...
      ctx.font = 'bold 11px sans-serif'; ctx.textAlign = 'center'; ctx.fillStyle = COLORS.white;
      ctx.globalAlpha = 0.7; ctx.fillText(`#${t.order}`, t.x, t.y + t.ry + 14); ctx.globalAlpha = 1;
    }
    ...
    for (const p of s.particles) {
      if (!p.active) continue; const a = p.life / p.maxLife;
      if (p.type === 'line') {
    ...
    ctx.font = '10px sans-serif'; ctx.fillStyle = COLORS.gold; ctx.fillText(`Goal: ${TARGET_TEMP}\u00b0`, gx, gt + 4);
    ...
    ctx.fillStyle = s.timeLeft < 5 ? COLORS.red : COLORS.cyan; ctx.fillRect(0, 0, w * (s.timeLeft / GAME_DURATION), 4);
    ctx.font = 'bold 24px sans-serif'; ctx.textAlign = 'right';
    ctx.fillStyle = s.timeLeft < 5 ? COLORS.red : COLORS.white; ctx.fillText(`${Math.ceil(s.timeLeft)}s`, w - 20, 40);
  ...
  useEffect(() => {
    if (phase === 'playing') {
      const s = state.current;
      s.toucans = makeToucans(); s.temperature = 18; s.timeLeft = GAME_DURATION;
      s.placedCount = 0; s.dragging = null;
      s.windTimer = WIND_INTERVAL; s.windActive = false; s.windAlpha = 0;
      s.wrongFlash = 0; s.correctFlash = 0; s.layoutDone = false;
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#000' }}>
      ...
      {phase === 'ended' && (
        <div style={{
          ...
          <div style={{ color: COLORS.gold, fontSize: 20, marginBottom: 8 }}>{state.current.placedCount}/{NUM_TOUCANS} toucans placed</div>
          <div style={{ color: COLORS.cyan, fontSize: 18, marginBottom: 8 }}>Temperature: {Math.round(state.current.temperature)}&deg;C</div>
